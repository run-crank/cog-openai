...
describe('CompletionReadability', () => {
  ...
  describe('Metadata', () => {
    ...
    it('should return expected step metadata', () => {
      ...
      expect(stepDef.getExpression()).to.equal('OpenAI model (?<model>[a-zA-Z0-9_-.]+) school level of the response to "(?<prompt>[a-zA-Z0-9_ -\p{P}]+)" should (?<operator>be less than|be greater than|be one of|be|not be one of|not be) ?(?<schoollevel>.+)?');
