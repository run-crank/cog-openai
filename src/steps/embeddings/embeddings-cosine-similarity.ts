...
export class EmbeddingsCosineSimilarity extends BaseStep implements StepInterface {
  ...
  protected stepExpression: string = 'OpenAI model (?<model>[a-zA-Z0-9_-]+) cosine similarity of (?<text1>[a-zA-Z0-9_-]+) (?<text2>[a-zA-Z0-9_-]+) should (?<operator>be set|not be set|be less than|be greater than|be one of|be|contain|not be one of|not be|not contain|match|not match) ?(?<semanticsimilarity>.+)?';
