...
export class CompletionResponseTime extends BaseStep implements StepInterface {
	protected stepName: string =
		"Check OpenAI GPT prompt response response time from request to completion in milliseconds";

	// tslint:disable-next-line:max-line-length
	protected stepExpression: string =
		'OpenAI model (?<model>[a-zA-Z0-9_-]+) response time in a response to "(?<prompt>[a-zA-Z0-9_ -]+)" should (?<operator>be set|not be set|be less than|be greater than|be one of|be|contain|not be one of|not be|not contain|match|not match) ?(?<expectation>.+)? ms';
	...
	protected expectedFields: Field[] = [
		...
		},
		{
			...
			description:
				"Check Logic (be, not be, be greater than, be less than, be set, not be set, be one of, or not be one of)",
		},
		{
			field: "expectation",
